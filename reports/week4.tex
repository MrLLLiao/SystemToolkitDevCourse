...
在实验十四中，Makefile 的配方行必须使用 Tab 而非空格缩进。在 Windows 编辑器中保存时，有时会将 Tab 转换为空格，导致 make 报错"missing separator"。解决方法是在 WSL 中使用 cat -A 检查 Makefile 中的缩进字符，确保配方行以 \texttt{<TAB>} 开头。
