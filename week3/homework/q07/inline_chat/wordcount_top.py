#!/usr/bin/env python3
"""wordcount 模式3（内联聊天）版：把聊天里 AI 给的片段原样应用到基线后。

基线文件一字未动，只有两处按 ai_reply.txt 的片段插入：
① 参数声明 ap.add_argument("--top", ...)
② main() 末尾的 if args.top: ... 块
"""
import argparse
import sys
